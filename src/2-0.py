...
# screen.subpad()は作れない。おそらくscreenはwindowであり、windowからはsubwin()しかできないため。
class Main:
    def __init__(self, screen, msg, color_index=1):
        self.__screen = screen
        self.__msg = msg
        self.__color_index = color_index
        self.__subpad = self.__screen.subpad(curses.LINES, curses.COLS)
        self.__init_cursor()
    ...
    def __draw(self):
        ...
        if self.__subpad.is_wintouched: self.__subpad.refresh(0, 0, 0, 0, curses.LINES-2, curses.COLS-2)
    def __input(self):
