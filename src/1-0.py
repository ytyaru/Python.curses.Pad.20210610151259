...
# curses.newpad()を使う。
# 注意: 
#   1. screen.refresh(), screen.getkey()を使うとダメ。上書きされてしまって何も表示されなくなる。pad.refresh(), pad.getkey()を使うべき。
#   2. self.__pad.keypad(True)を使う。さもなくばキー入力ができない。
class Main:
    ...
    def __draw(self):
        ...
        self.__pad.refresh(self.__y, self.__x, 0, 0, curses.LINES-1, curses.COLS-1)
#        self.__pad.refresh(0, 0, 0, 0, curses.LINES-1, curses.COLS-1)
    def __input(self):
        ...
        while True:
            ...
            elif curses.KEY_DOWN == key: self.__y += 1 if self.__y < self.__pad.getmaxyx()[0] - curses.LINES else 0
            elif curses.KEY_LEFT == key: self.__x -= 1 if 0 < self.__x else 0
            elif curses.KEY_RIGHT == key: self.__x += 1 if self.__x < self.__pad.getmaxyx()[1] - curses.COLS else 0
            else: break
            self.__pad.refresh(self.__y, self.__x, 0, 0, curses.LINES-1, curses.COLS-1)
